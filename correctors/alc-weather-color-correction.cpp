...
namespace Correctors {
	...
	ALCWeatherColorCorrection::ALCWeatherColorCorrection(QObject *parent) :
		QObject(parent),
		Correctors::ALCColorCorrection(Type::Weather) {
	...
	void ALCWeatherColorCorrection::callCorrection(const QTime &current) {
		...
		setCorrection(Color::Brightness, clock);
		qDebug() << "Current time:" << current << ", global: "  << correction(Color::Brightness, true) << "scalar: " << clock;
	}
