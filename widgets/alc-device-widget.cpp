...
void ALCDeviceWidget::deviceConnected(DeviceThread *) {
...
void ALCDeviceWidget::deviceDisconnected(DeviceThread *thread) {
	for (int i = 0; i < m_devices.count(); ++i) {
		if (thread == dynamic_cast<DeviceThread *>(m_devices[i]->receiver())) {
			delete m_devices[i];
...
void ALCDeviceWidget::addWorkspace(ALCDeviceTreeWidget *item, DeviceThread *thread) {
